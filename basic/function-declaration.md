...
しかし、関数の内部では`arguments`という変数で、実際に渡された引数を配列のように参照できる。
...
このように関数が値として扱えることを、ファーストクラスファンクション（第一級関数）と呼ぶ。
...
以下のようにアロー関数で`const`や`let`を使って定義すると、同じ変数名を定義できないため、構文エラーとなる。
